...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        #print("stateState=",self.startingPosition)
        return (self.startingPosition,self.startFound)
            ...
            if not hitsWall:
                ...
                for i in range(0,4):
                    if nextPos == self.corners[i]:
                        newFounded = founded[0:i]+(1,)+founded[i+1:]
                        break
                successors.append(((nextPos,newFounded),action,1))
...
def cornersHeuristic(state, problem):
    ...
    if False:
        ...
        for j in range(0,tot):
            ...
            for i in range(0,4):
                corner=corners[i]
                if founded[i]==0 and not vis[i]:
                    if abs(corner[0]-x)+abs(corner[1]-y)<d:
...
def foodHeuristic(state, problem):
    ...
    for i in range(0,n):
        if not vis[i]:
           dx,dy=fl[i]
           dist = mazeDistance((x,y),(dx,dy),problem.startingGameState)
           if dist > maxDist:
               maxDist = dist
               maxIdx = i
